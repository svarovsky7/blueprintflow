  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const hasUnit = row.length >= 3
        const unitName = hasUnit && row[1] != null ? String(row[1]).trim() : ''
        const quantityCell = hasUnit ? row[2] : row[1]
        const quantity = quantityCell != null && quantityCell !== '' ? Number(quantityCell) : null
