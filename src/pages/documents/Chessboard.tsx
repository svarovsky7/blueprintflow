...
export default function Chessboard() {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const payload: { project_id: string; material: string; quantityRd: number | null }[] = []
      for (let i = 1; i < rows.length; i++) {
        const row = rows[i]
        const material = (row[0] as string | undefined)?.trim()
        const quantity = row[1] as string | number | undefined
  ...
  }, [importFile, importState, message, refetch])
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Select
            ...
            onChange={(value) =>
              setImportState((s) => ({ ...s, categoryId: value || undefined, typeId: undefined }))
            }
            options={[
              { value: '', label: 'НЕТ' },
              ...(costCategories?.map((c) => ({
          ...
          <Select
            ...
            onChange={(value) =>
              setImportState((s) => ({ ...s, typeId: value || undefined }))
            }
            options={[
              { value: '', label: 'НЕТ' },
              ...(costTypes
